...
// Importante: NÃO desestruturar aqui (const { userDB } = ...). Precisamos do
// objeto "database" inteiro, para poder alterar database.userDB.usuarios
// diretamente. Se desestruturássemos e reatribuíssemos userDB, o
// userService.js continuaria enxergando os dados antigos, porque cada
// arquivo teria sua própria referência.
...
server.post('/cadastrar', async (req, res) => {
  ...
  // Gera o hash da senha antes de salvar. Nunca salve senha em texto puro!
  const senhaHash = await bcrypt.hash(senha, 10);

  const usuarios = database.userDB.usuarios;
  const last_item_id = usuarios.length > 0 ? usuarios[usuarios.length - 1].id : 0;

  // Damos push no MESMO array (mutação), em vez de criar um objeto novo.
  // Assim, o userService.js (que usa a mesma referência) já enxerga o
  // usuário recém-cadastrado, sem precisar reiniciar o servidor.
  usuarios.push({ id: last_item_id + 1, email, senha: senhaHash, nome, endereco, complemento, cep });

  // Persiste a alteração no arquivo em disco.
  database.writeJsonFile('./src/data/usuarios.json', database.userDB);
...
server.post('/login', async (req, res) => {
  ...
  const usuarioValido = await usuarioExiste(email, senha);
  ...
  const access_token = createToken({ email })
  let user = { ...database.userDB.usuarios.find(user => user.email === email) }
  delete user.senha
  ...
  res.status(200).json([
    {
      ...
      "autor": 4,
      "opcoesCompra": [
        { "id": 1, "titulo": "E-book", "preco": 29.9, "formatos": [".pdf", ".pub", ".mob"] },
        { "id": 2, "titulo": "Impresso", "preco": 39.9 },
        { "id": 3, "titulo": "E-book + Impresso", "preco": 59.9, "formatos": [".pdf", ".pub", ".mob"] }
      ],
